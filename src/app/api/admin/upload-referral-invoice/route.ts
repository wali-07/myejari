...
/**
 * POST /api/admin/upload-referral-invoice
 *
 * Multipart body:
 *   `invoice`          — the order invoice number (e.g. "INV0123")
 *   `referralInvoice`  — the invoice the referral partner shared (PDF or image)
 *
 * Saves through the storage abstraction (fs in dev, Blob in prod) and
 * writes the resulting ref back to the order's `referralInvoicePath`.
 */
export async function POST(request: Request) {
  ...
  const file = form.get("referralInvoice");
  if (!invoice) {
    return NextResponse.json(
  ...
  if (!(file instanceof File) || file.size === 0) {
    return NextResponse.json(
      { error: "referralInvoice file is required" },
      { status: 400 }
    );
  }
  ...
  // Replace any existing referral invoice for this order.
  await deleteUploadsByPrefix(`orders/${invoice}/referral-invoice`);

  const buffer = Buffer.from(await file.arrayBuffer());
  const key = `orders/${invoice}/referral-invoice${ext}`;
  ...
  orders[idx] = { ...orders[idx], referralInvoicePath: stored.ref };
